    ...
    async def _recv_loop(self, conn):
        async for raw in conn:
            ...
            if etype == "aggTrade":
                ...
                # Progress any tracked AI trade for this symbol on every live
                # tick — fills, TP1/TP2, stop hits and breakeven trailing
                # happen here with no AI call required.
                trade_update = ai_analyst.tracker.update_price(symbol, float(msg["p"]))
                if trade_update:
                    for c in self.clients:
                        if c.symbol == symbol:
                            c.send({"type": "trade", "data": trade_update})
            elif etype == "kline":
                k = msg["k"]
