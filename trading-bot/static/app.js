...
(function () {
  ...
  function onAI(a) {
    ...
    // Only toast a fresh signal card when a NEW call is armed (mode
    // "prospect"). Ongoing "manage" checks on an already-tracked trade must
    // not spam a new card every refresh cycle — that's the old robot behavior.
    if (a.mode !== "prospect") return;
    var key = a.symbol + ":" + (a.trade ? a.trade.armed_at : a.entry);
  ...
  // ---------- tracked trade lifecycle panel ----------
  var STATUS_LABEL = {
    ARMED: "Armed \u00b7 waiting for fill", OPEN: "Open \u00b7 managing",
    CLOSED_WIN: "Closed \u00b7 win", CLOSED_LOSS: "Closed \u00b7 loss",
    CLOSED_BE: "Closed \u00b7 breakeven", INVALIDATED: "Invalidated",
    EXPIRED: "Expired \u00b7 never filled",
  };
  var STATUS_CLASS = {
    ARMED: "armed", OPEN: "open", CLOSED_WIN: "win",
    CLOSED_LOSS: "loss", CLOSED_BE: "loss", INVALIDATED: "invalid", EXPIRED: "invalid",
  };

  function onTrade(t) {
    var card = document.getElementById("trade-card");
    if (!t || t.symbol !== symbolEl.value) {
      if (!t) card.classList.add("hidden");
      return;
    }
    card.classList.remove("hidden");
    var digits = digitsFor(t.entry || t.last_price || 1);
    var status = document.getElementById("trade-status");
    status.textContent = t.signal + " \u00b7 " + (STATUS_LABEL[t.status] || t.status);
    status.className = "trade-status " + (STATUS_CLASS[t.status] || "");
    document.getElementById("trade-entry").textContent = fmt(t.entry, digits);
    document.getElementById("trade-stop").textContent = fmt(t.stop, digits);
    document.getElementById("trade-tp1").textContent = fmt(t.tp1, digits) + (t.tp1_hit ? " \u2713" : "");
    document.getElementById("trade-tp2").textContent = fmt(t.tp2, digits);
    var notesHost = document.getElementById("trade-notes");
    notesHost.innerHTML = "";
    (t.notes || []).slice(-5).reverse().forEach(function (n) {
      var row = document.createElement("div");
      row.className = "tn-row";
      var when = new Date(n.t * 1000).toLocaleTimeString();
      row.innerHTML = '<span class="tn-time">' + when + "</span><span>" + n.text + "</span>";
  ...
  function reset() {
    ...
    document.getElementById("trade-card").classList.add("hidden");
    lastFlashPrice = null;
    shownPrice = null;
    targetPrice = null;
    priceEl.textContent = "\u2014";
